...
typeof(e1) #type & class are environment
class(e1)
```
...
```{r}
e1$d <- e1 #this changes the letter d to an environment aka copy of e1
```
...
```{r}
env_names(e1) #name in this environment
names(e1)
```
...
**Can see global environment in one of the panes. For example, mine is on the upper right hand pane.**
