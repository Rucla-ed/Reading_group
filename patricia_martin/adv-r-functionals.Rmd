...
```{r}
randomise <- function(f) f(runif(1e3)) #f is the attribute which represents a function like mean, sum, etc.
randomise(mean)
randomise(mean)
randomise(sum)
```
...
```{r}
triple <- function(x) x * 3 #function that multiplies value by 3
map(1:3, triple) #map takes a vector (e.g. 1:3) and a function "triple". Essentially the map function is multiplying the each element of the vector 1:3 by 3. 
```
...
```{r}
by_cyl <- split(mtcars, mtcars$cyl) #this splits the data into groups (3 cylinders)
```
...
```{r}
by_cyl %>% #call list
  map(~ lm(mpg ~ wt, data = .x)) %>% #run linear model
  map(coef) %>% #grab coefficient
  map_dbl(2) #grab second coefficient
```
